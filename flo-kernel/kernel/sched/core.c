...
int nr_cpus_foreground = NR_CPUS / 2;
...
SYSCALL_DEFINE2(sched_set_cpugroup, int, numcpu, int, group)
{
	...
	if ((group != FOREGROUND && group != BACKGROUND) ||
	    numcpu < 1 || numcpu > NR_CPUS - 1)
		return -EINVAL;

	old_ncf = nr_cpus_foreground;
	switch (group) {
	case FOREGROUND:
		nr_cpus_foreground = numcpu;
		break;
	case BACKGROUND:
		nr_cpus_foreground = NR_CPUS - numcpu;
		break;
	default:
		BUG_ON(group);
	}
	if (nr_cpus_foreground > old_ncf) {
		start_cpu = old_ncf;
	...
	for (i = start_cpu; i < end_cpu; i++) {
		struct list_head *head = &cpu_rq(i)->grr.head.run_list;
		struct sched_grr_entity *iter;

		list_for_each_entry(iter, head, run_list) {
			struct task_struct *task;
			int new_cpu;

			task = container_of(iter, struct task_struct, grr);
			deactivate_task(cpu_rq(i), task, 0);
			new_cpu = select_task_rq(task, SD_BALANCE_WAKE, 0);
			activate_task(cpu_rq(new_cpu), task, 0);
		}
	}
	return 0;
}
